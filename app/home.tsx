...
const Home: React.FC = () => {
    ...
    const handleUploadTape = async () => {
        try {
            ...
            if (!result.canceled && result.assets && result.assets[0]) {
                ...
                const response = await fetch("http://35.2.11.129:8000/upload-video", {
                    method: "POST",
                    body: formData,
                    headers: {
            ...
            onSave: async (updatedData: any) => {
                ...
                try {
                    ...
                    const response = await fetch(
                        `http://35.2.11.129:8000/save-processed-result/${reviewFileName}`,
                        {
                            method: "POST",
                            headers: {
    ...
    const handleUnpublishedTapes = async () => {
        try {
            const response = await fetch("http://35.2.11.129:8000/processed-results");
            if (!response.ok) {
                            ...
                            navigation.navigate("Review", {
                                ...
                                onSave: async (updatedData: any) => {
                                    ...
                                    try {
                                        // Extract directory name from reviewFilePath
                                        const response = await fetch(
                                            `http://35.2.11.129:8000/save-processed-result/${tape.name}`,
                                            {
                                                method: "POST",
                                                headers: {
                                                    "Content-Type": "application/json",
                                                ...
                                                [
                                                    { text: "Cancel", style: "cancel" },
                                                    {
                                                        ...
                                                        onPress: async (township?: string) => {
                                                            ...
                                                            try {
                                                                const response = await fetch(
                                                                    `http://35.2.11.129:8000/move-to-polltickets/${tape.name
                                                                    }/${encodeURIComponent(
                                                                        county
